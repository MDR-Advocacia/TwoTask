...
class ClassifUploadResponse(BaseModel):
    ...
    updated: int
    skipped: list[dict]
    deleted: list[dict] = Field(
        default_factory=list,
        description=(
            "Itens hard-deleted pelo modo absoluto (origem=planilha em "
            "status pendente/processando/erro que sumiram da nova planilha). "
            "Cada entrada: {id, cnj, status_anterior}."
        ),
    )
...
async def upload_classif_xlsx(
    ...
    sync_mode: bool = Query(
        default=True,
        description=(
            "Quando True (default), apos upsert faz HARD DELETE dos itens "
            "com origem='planilha' em status pendente/processando/erro "
            "que NAO estao na nova planilha. Itens com status='sucesso' "
            "sao preservados (historico). Itens com origem='intake_auto' "
            "sao preservados (outro fluxo). Set False pra upload incremental."
        ),
    ),
    db: Session = Depends(get_db),
    _: LegalOneUser = Depends(auth_security.require_permission("prazos_iniciais")),
    ...
    Modo absoluto (sync_mode=True, default): a planilha é a fonte de
    verdade do que está pendente no AJUS. Itens da fila com
    origem='planilha' que sumiram da nova planilha são apagados.
    """
    try:
    ...
    result = AjusClassificacaoService(db).enqueue_from_xlsx_rows(
        rows, sync_mode=sync_mode,
    )
