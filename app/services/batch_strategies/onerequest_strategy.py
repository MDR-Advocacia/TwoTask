...
from app.services.mail_service import send_failure_report  # <--- Importação do novo serviço
...
class OnerequestStrategy(BaseStrategy):
    ...
    async def process_batch(self, request: BatchTaskCreationRequest, execution_log: BatchExecution) -> dict: 
        ...
        # --- ENVIO DE E-MAIL DE ALERTA (Se houver falhas) ---
        if failed_items:
            logging.info(f"Detectadas {len(failed_items)} falhas. Iniciando envio de alerta por e-mail...")
            try:
                ...
                logging.error(f"Erro ao tentar enviar e-mail de falha: {e}")
        # ----------------------------------------------------
