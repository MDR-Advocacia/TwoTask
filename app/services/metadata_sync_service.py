...
class MetadataSyncService:
    ...
    async def sync_all_metadata(self):
        ...
        try:
            ...
            await self._sync_users() # Adicionada a sincronização de usuários
    ...
    async def _sync_task_types(self):
        ...
        # Primeira Passada: Tipos (pais)
        for type_data in api_types:
            external_id = self._to_int(type_data.get("id"))
            if not external_id: continue
            existing_type = existing_types_map.get(external_id)
            if not existing_type:
                new_type = LegalOneTaskType(external_id=external_id, name=type_data.get("name"), parent_id=None)
                self.db.add(new_type)
                existing_types_map[external_id] = new_type

        self.db.commit()

        # Segunda Passada: Subtipos (filhos)
        ...
        for subtype_data in api_subtypes:
            ...
            if not parent_internal_id: continue
            ...
            if existing_subtype:
                ...
                new_subtype = LegalOneTaskType(external_id=external_id, name=subtype_data.get("name"), parent_id=parent_internal_id)
                self.db.add(new_subtype)
        
        self.db.commit()
        self.logger.info("Sincronização de Tipos e Subtipos de Tarefa concluída.")

    # --- NOVO MÉTODO PARA SINCRONIZAR USUÁRIOS ---
    async def _sync_users(self):
        """
        Busca os usuários da API e realiza o 'upsert' na tabela local
        `legal_one_users`.
        """
        self.logger.info("Sincronizando Usuários...")
        ...
        self.db.commit()
        self.logger.info(f"Sincronização de Usuários concluída. Adicionados: {count_add}, Atualizados: {count_update}.")